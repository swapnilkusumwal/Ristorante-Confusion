...
export const postCart = (cart) => (dispatch) => {
  ...
  return fetch(baseUrl + 'cart', {
      method: "POST",
      body: JSON.stringify({"dishid":cart.dishid,"name":cart.name,"price":cart.price,"quantity":cart.quantity,image:cart.image}),
      headers: {
        "Content-Type": "application/json",
        'Authorization': bearer
      },
...
export const deleteCart = (dishId) => (dispatch) => {
  ...
  .then(response => response.json())
  .then(cart => { console.log('Cart Deleted', cart); dispatch(addCart(cart)); })
  .catch(error => dispatch(cartFailed(error.message)));
};
